...
# ### Check global means.
print("global mean Ts in Earth coords = %.3fK" % np.average(np.average(getattr(state,var),axis=-1) * state.weights,axis=-1))
print("global mean Ts in TL coords = %.3fK" % np.nanmean(np.nanmean(getattr(state_TL,var),axis=-1) * state_TL.weights,axis=-1))
...
def plot_var(var="TS", ind_p = -1, n = 2, units=None):
    ...
    plt.savefig(f"{var}_TL_{alt}.pdf",format='pdf')


# ### Make a plot of surface temp, overlay surface winds
def plot_winds(var="TS", ind_p = -1, n = 4, units=None):
    # ind_p : desired index in vertical, e.g., mid-troposphere or near surface
    # n : how dense to plot wind vectors?
    try:
        alt = f"{state.alt[ind_p]:.1f}"
    ...
    plt.tight_layout()
    plt.savefig(f"{var}_wind_TL_{alt}.pdf",format='pdf')

plot_var("TS", units="K", ind_p=-1)
plot_var("CLDTOT", ind_p=-1, )
plot_winds("TS", ind_p=-1)